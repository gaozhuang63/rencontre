...
Page({
      ...
      success: function (res) {
        ...
        if (that.data.isFilmFavorite) {
          ...
          for (var i = 0; i < film_favorite.length; i++) {
            if (film_favorite[i].id == that.data.ticketDetail.id) {
          ...
          // 添加
          film_favorite.push(that.data.ticketDetail)
          wx.setStorage({
            key: 'film_favorite',
            data: film_favorite,
            success: function (res) {
  ...
  getticket: function () {
    ...
    wx.getStorage({
      key: 'ticket_getting',
      success: function (res) {
        var ticket_getting = res.data
        if (that.data.isticketgetting) {
          // 删除
          for (var i = 0; i < ticket_getting.length; i++) {
            if (ticket_getting[i].id == that.data.ticketDetail.id) {
              ticket_getting.splice(i, 1)
              that.setData({
                isticketgetting: false
              })
            }
          }
          wx.setStorage({
          ...
          // 添加
          ticket_getting.push(that.data.ticketDetail)
          wx.setStorage({
            key: 'ticket_getting',
            data: ticket_getting,
            success: function (res) {
